...
export default function LaunchWidget() {
  ...
  const startPolling = () => {
    if (!serverOn) {
      axios
        .get(`${process.env.REACT_APP_API_HOST}/api/ping`)
        .then((result) => {
          setServerOn(true);
        })
        .catch((error) => {
  ...
  const connect = new Connect({
    ...
    onSuccess: ({ code }) => {
      ...
      axios
        .get(
          `${process.env.REACT_APP_API_HOST}/api/exchangetoken?token=${code}&country=${country}`
        )
        .then((result) => {
  ...
  if (accountId != "") {
    return (
      <div className="App">
        <h3 className="title">Mombasa</h3>

        <h4 className="subtitle">
          Congrats! You've linked your account to Mono.
        </h4>

        <AccountBox token={token} accountId={accountId} />

        <p className="description description-extra-margin">
          Now that you have an account id, you can make all of the following
          requests:
        </p>
    ...
    return (
      <div className="App">
        ...
        {/* <p className="description">
          The Mono Connect flow begins when your user wants to connect their
          bank account to your app. Simulate this by clicking the button below
          to launch Connect - the client-side component that your users will
          interact with in order to link their accounts to Mono and allow you to
          access their accounts via the Mono Connect API.
        </p> */}
